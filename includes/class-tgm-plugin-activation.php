	...
	class TGM_Plugin_Activation {
		...
		protected function add_admin_menu( array $args ) {
			if ( has_filter( 'tgmpa_admin_menu_use_add_theme_page' ) ) {
				_deprecated_function( 'The "tgmpa_admin_menu_use_add_theme_page" filter', '2.5.0', esc_html__( 'Set the parent_slug config variable instead.', 'neat' ) );
			}
		...
		public function show_tgmpa_version() {
			echo '<p style="float: right; padding: 0em 1.5em 0.5em 0;"><strong><small>',
				esc_html( sprintf( _x( 'TGMPA v%s', '%s = version number', 'neat' ), self::TGMPA_VERSION ) ),
				'</small></strong></p>';
		}
	...
	function tgmpa( $plugins, $config = array() ) {
		...
		if ( ! empty( $config ) && is_array( $config ) ) {
			...
			if ( isset( $config['notices'] ) ) {
				_deprecated_argument( __FUNCTION__, '2.2.0', esc_html__( 'The `notices` config parameter was renamed to `has_notices` in TGMPA 2.2.0. Please adjust your configuration.', 'neat' ) );
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		 ...
		 */
		public function __construct() {
			$this->tgmpa = call_user_func( array( get_class( $GLOBALS['neat'] ), 'get_instance' ) );
		...
		public function no_items() {
			printf( wp_kses_post( __( 'No plugins to install, update or activate. <a href="%1$s">Return to the Dashboard</a>', 'neat' ) ), esc_url( self_admin_url() ) );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		}
		...
		public function get_bulk_actions() {
			...
			if ( 'update' !== $this->view_context && 'activate' !== $this->view_context ) {
				if ( current_user_can( 'install_plugins' ) ) {
					$actions['tgmpa-bulk-install'] = esc_html__( 'Install', 'neat' );
				}
			}

			if ( 'install' !== $this->view_context ) {
				if ( current_user_can( 'update_plugins' ) ) {
					$actions['tgmpa-bulk-update'] = esc_html__( 'Update', 'neat' );
				}
				if ( current_user_can( 'activate_plugins' ) ) {
					$actions['tgmpa-bulk-activate'] = esc_html__( 'Activate', 'neat' );
				}
			}
...
if ( ! function_exists( 'tgmpa_load_bulk_installer' ) ) {
	...
	function tgmpa_load_bulk_installer() {
		...
		$tgmpa_instance = call_user_func( array( get_class( $GLOBALS['neat'] ), 'get_instance' ) );
